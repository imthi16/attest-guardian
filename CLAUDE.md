...
make contracts             # regenerate packages/contracts/openapi.json (the web/API contract)
...
The request flow, all of which is built. Every stage below runs today; the detail for each
subsystem follows:
...
- `services/` — documentation-only boundary markers for `ingestion`, `retrieval`, `verification`,
  and `safety`. **No code lives here**: each is implemented as a package under `apps/api/app/`
  (`ingestion/`+`parsing/`+`chunking/`, `retrieval/`+`embeddings/`+`reranking/`,
  `verification/`+`decision/`+`citations/`, `safety/`). The directories mark the seams a package
  would be extracted along if it ever needed to scale, deploy, or fail independently — splitting
  before that need is real buys a network hop and loses a transaction. Put new code in
  `apps/api/app/`, and keep each boundary enforcing its own authorization and input-trust rules
  there.
- `packages/contracts/openapi.json` — the API's own description of itself, generated by
  `app/contracts.py` (`make contracts`) and committed. It is the pin between two programs
  that must agree: `apps/api/tests/test_contracts.py` fails if it drifts from the
  application, `apps/web/lib/contracts.drift.test.ts` fails if a Zod schema has a field
  the API does not return (or lacks one it does), and `attest-api.drift.test.ts` fails if
  the app requests a path the API does not serve. Regenerate deliberately and read the
